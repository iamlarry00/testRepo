...
class Minify_HTML {
	...
	### 내용중 이미지 url만 추출한다###
	public static function getValue_array($contents, $key, $val){
		...
		if($match[0]){
			...
			foreach($match[0] as $k => $v){
				...
				$v2 = str_replace("src='","", $v);
				$v2 = str_replace('src="',"", $v2);
				$v2 = str_replace('"',"", $v2);
				$v2 = str_replace("'","", $v2);
				...
				### 확장자 추출
				$filename = explode(".", $v2);
				$this_ext = $filename[count($filename)-1];
				$this_ext = strtolower($this_ext); //확장자를 소문자로...

				if(in_array($this_ext, array("jpg","gif","png"))){
